  ...
  return (
    <section className="flex flex-col items-center justify-center h-full py-8">
      <div className="md:w-1/2 text-center p-4">
          ...
          <div className="flex items-center justify-center">
            {
                loginError && (
                    <div>
                      <span className="p-2 text-xl text-red-600 font-semibold cursor-default"> {loginError} </span>
                    </div>
                )
            }
          </div>
        ...
        <Link href="/auth/login" className ="text-xl hover:text-button-hover hover:text-blue-600 cursor-pointer">Already have an account? Log in</Link>
      </div>
    </section>
  );
}
